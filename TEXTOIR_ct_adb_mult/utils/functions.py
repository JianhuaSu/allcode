...
import logging
import copy
from .metrics import Metrics, OOD_Metrics, OID_Metrics
...
def softmax_cross_entropy_with_softtarget(input, num_labels, device):
    ...
    # batchloss = - torch.sum(ood_targets.unsqueeze(1) * logprobs, dim=1)
...
def save_results(args, test_results, debug_args = None):
    ...
    _vars = [args.dataset, args.ood_dataset, args.method, args.ood_detection_method, args.text_backbone, args.seed, args.log_id]
    _names = ['dataset', 'ood_dataset', 'method', 'ood_detection_method', 'text_backbone', 'seed', 'log_id']
