...
class MULT(nn.Module):
    ...
    def forward(self, text_feats, video_feats, audio_feats, binary_inputs = None, feature_ext = False):
        
        text = self.text_subnet(text_feats)

        if feature_ext:
            return text, video_feats, audio_feats

        x_l = F.dropout(text.transpose(1, 2), p=self.text_dropout, training=self.training)
        x_a = audio_feats.transpose(1, 2).float()
        x_v = video_feats.transpose(1, 2).float()
        ...
        return logits, last_hs_proj

    def vim(self):

        return self.out_layer.weight, self.out_layer.bias