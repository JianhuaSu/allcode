from moviepy.editor import *
from tqdm import tqdm
from transformers import AutoProcessor, WavLMModel, HubertModel
import os
import argparse
import pickle
import argparse
import librosa
import numpy as np
import logging
import torch

__all__ = ['AudioFeature']

audio_pretrained_model_path = {
    'wav2vec2.0': 'facebook/wav2vec2-base-960h',
    'WavLM': 'patrickvonplaten/wavlm-libri-clean-100h-base-plus',
    'HuBERT': 'facebook/hubert-large-ls960-ft'
}

def parse_arguments():
    
    parser = argparse.ArgumentParser()
    parser.add_argument('--raw_video_path', type=str, default='/root/autodl-tmp/demo/datasets/video_data/raw_data', help="The directory of the raw video path.")
    parser.add_argument('--audio_data_path', type=str, default='/root/autodl-tmp/demo/datasets/audio_data', help="The directory of the audio data path.")
    parser.add_argument('--raw_audio_path', type=str, default='/root/autodl-tmp/demo/datasets/audio_data/raw_audio', help="The directory of the raw audio path.")
    parser.add_argument("--audio_feats_path", type=str, default='audio_feats.pkl', help="The directory of audio features.")

    args = parser.parse_args()

    return args

class AudioFeature:
    
    def __init__(self, args, get_raw_audio = False):


        self.processor = AutoProcessor.from_pretrained('/root/autodl-tmp/demo/mode/wavlm-libri-clean-100h-base-plus')
        self.model = WavLMModel.from_pretrained('/root/autodl-tmp/demo/mode/wavlm-libri-clean-100h-base-plus')
            
        self.sr = args.sr

        if get_raw_audio:
            self.__get_raw_audio(args)
    
        audio_feats = self.__gen_feats_from_audio(args, use_pretrained_model=True)
        self.__save_audio_feats(args, audio_feats)

    def __get_raw_audio(self, args):
        raw_audio_path = os.path.join(args.audio_data_path, args.raw_audio_path)

        if not os.path.exists(raw_audio_path):
            os.makedirs(raw_audio_path)       

        # 直接遍历 raw_video_path 下的所有文件
        for file in tqdm(os.listdir(args.raw_video_path), desc = 'Files'):
            file_path = os.path.join(args.raw_video_path, file)

            # 确保只处理视频文件（例如 .mp4 文件）
            if os.path.isfile(file_path) and file.endswith('.mp4'):
                video_name = file.split('.')[0]  # 去掉扩展名获取视频名
                audio_data_path = raw_audio_path

                if not os.path.exists(audio_data_path):
                    os.makedirs(audio_data_path)

                # 检查是否已经存在音频文件，若存在则跳过
                if os.path.exists(os.path.join(audio_data_path, video_name + ".wav")):
                    continue

                # 提取音频并保存
                video_segments = VideoFileClip(file_path)
                audio = video_segments.audio
                audio.write_audiofile(os.path.join(audio_data_path, video_name + ".wav"))

        
    def __gen_feats_from_audio(self, args, use_pretrained_model=False):
        audio_feats = {}
        raw_audio_path = os.path.join(args.audio_data_path, args.raw_audio_path)

        # 遍历 raw_audio_path 中的所有文件
        for file in tqdm(os.listdir(raw_audio_path), desc = 'Audio Files'):
            file_path = os.path.join(raw_audio_path, file)

            # 仅处理音频文件（如 .wav 文件）
            if os.path.isfile(file_path) and file.endswith('.wav'):
                audio_id = file.split('.')[0]  # 使用文件名作为音频 ID

                # 生成音频特征
                if use_pretrained_model:
                    pretrained_model_feats = self.__process_audio(file_path)
                    audio_feats[audio_id] = pretrained_model_feats
                else:
                    mfcc = self.__process_audio(file_path)
                    audio_feats[audio_id] = mfcc

        return audio_feats


    def __process_audio(self, read_file_path):
        
        y, sr = librosa.load(read_file_path, sr = self.sr)
        audio_feats = self.processor(y, sampling_rate = 16000, return_tensors="pt").input_values
        with torch.no_grad():
            audio_feats = self.model(audio_feats).last_hidden_state.squeeze(0)
        
        return audio_feats

    def __save_audio_feats(self, args, audio_feats):

        audio_feats_path = os.path.join(args.audio_data_path, args.audio_feats_path)

        with open(audio_feats_path, 'wb') as f:
            pickle.dump(audio_feats, f)

if __name__ == '__main__':
    
    args = parse_arguments()


    args.audio_feats_path = 'wavlm_feats.pkl'
    args.audio_pretrained_model = 'WavLM'
    
    args.sr = 16000
    
    audio_data = AudioFeature(args, get_raw_audio = True)